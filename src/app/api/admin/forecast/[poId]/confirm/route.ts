...
import { confirmPO } from '@/controllers/forecastingController';
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ poId: string }> }) {
  ...
  try {
    ...
    console.error('[FORECAST CONFIRM ERROR]', err);
    return NextResponse.json({ error: 'Gagal konfirmasi PO.' }, { status: 500 });
  }
}
